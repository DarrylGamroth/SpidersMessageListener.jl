...
function Agent.on_start(agent::ControlAgent)
    ...
    try
        ...
        @error "Error starting agent $(Agent.name(agent)):" exception = (e, catch_backtrace())
    end
end

function Agent.on_close(agent::ControlAgent)
    println("Closing agent $(Agent.name(agent))")
...
function Agent.on_error(agent::ControlAgent, error::Exception)
    println("Error in agent $(Agent.name(agent)): $error")
    throw(error)
end
